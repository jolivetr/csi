...
class strainfield(SourceInv):
    ...
    def fault2Strain_finitediff(self, fault, s2m=1., p2m=1e3, la=30e9, mu=30e9, nu=0.25, slipdirection='sd', force_dip=None, verbose=False, dx=1.):
        '''
        ...
        assert fault.patchType in ('rectangle', 'triangle'), 'FD Strain calculation not implemented for the triangular tent case yet...'
        ...
        if fault.patchType=='rectangle':
            ...
            for ii, (patch, ss, ds, ts) in enumerate(zip(fault.patch, strikeslip, dipslip, tensileslip)):
                if verbose:
                    sys.stdout.write('\r Patch {} / {}'.format(ii, len(fault.patch)))
                    sys.stdout.flush()
                Strain[:,:,:] += triDisp.strain(self.x*p2m, self.y*p2m, -1.0*self.depth*p2m, list(patch*p2m), 
                                                ss, ds, ts, nu=nu)
